...
const SSR_ENABLED = process.env.SSR === 'false';
const DIST_FOLDER = join(process.cwd(), 'dist');

// * NOTE :: leave this as require() since this file is built Dynamically from webpack
const {AppServerModuleNgFactory, LAZY_MODULE_MAP} = require('./dist/server/main.bundle');
const viewEngine = SSR_ENABLED
  ? ngExpressEngine({
  ...
  : (_req, res, cb) => cb(null, res.document);
...
app.get('*', (req, res) => {
  res.render('index', {
    ...
    document: render(
      ...
      {
        // possible partial from external file, this can be populated via readFile
